...
import * as ds from './index_md.mjs'; // 注意：在运行时会引用编译后的.mjs文件

// 包含要处理的文本文件的文件夹
const sourceFolder: string = '../../ds-file/old'; // 更改为你的源文件夹路径
// 新文件将被保存的文件夹
const outputFolder: string = '../../ds-file/new'; // 更改为你的输出文件夹路径
// 要添加到每个文件的内容
const contentToAppend: string = 'xxxx'; // 更改为你想要添加的内容
...
// 递归处理目录和其子目录
async function processDirectory(sourcePath: string, relativePath: string = ''): Promise<void> {
  // 读取当前目录中的所有文件和文件夹
...
// 处理单个文本文件
async function processTextFile(sourceFilePath: string, relativeFilePath: string): Promise<void> {
  try {
    // 读取文件内容
    const data = fs.readFileSync(sourceFilePath, 'utf8');
    
    var result = await ds.callApi(data);
    // const result: string = ds.add();
...
try {
  console.log('开始处理文件...');
  processDirectory(sourceFolder).then(() => {
